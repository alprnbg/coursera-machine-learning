...
K = num_labels;
y_t = zeros(size(y,1),K);
for row = 1:size(y,1)
    y_t(row,y(row)) = 1;
end
X_t = [ones(size(X,1),1) X];    %5000x401
z2 = sigmoid(X_t * Theta1');    %5000x25
a2 = [ones(size(z2,1),1) z2];   %5000x26 
...
for k = 1:K
    J = J + (1/m)*((-1)*(y_t(:,k)')*log(h(:,k))-(1-y_t(:,k))'*log(1-h(:,k)));%  +  (lambda/(2*m))*(sum(theta(2:end).^2));
end
J = J + J_reg;
